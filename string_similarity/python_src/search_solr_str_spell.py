...
def search_solr_str_spell(query_str, query_spell, out_file, count=100):
    """
    ...
    url_dict["rows"] = count
    url_dict["title"] = query_str
    url_dict["title_spell"] = query_spell
    url = url_body + str_escape_solr(url_dict["title_spell"]) + ")OR(title:" + str_escape_solr(url_dict["title"]) + ")&start=0&rows=" + str(url_dict["rows"])
    # print url
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        print 'Usage: %s in_str in_spell out_file [count]' % sys.argv[0]
        sys.exit(1)
    ...
    # search
    result = search_solr_str_spell(in_str, in_spell, out_file, count)
    print result
